...
using namespace std;
double
drand (double lo, double hi)
{
  return lo + rand () / (double) RAND_MAX *(hi - lo);
}
__global__ void
function_on_GPU_a5 (float *a2_dev, float *a3_dev, float *a4_dev)
{
  ...
  for (int a24 = 0; a24 < 655360; ++a24) {
	...
	(((a12_sha + (2.0 * a15_sha)) + (2.0 * a18_sha)) + a21_sha)));
    a3_sha =
      (a10_sha +
       (1.6666666666666666e-3 *
	(((a13_sha + (2.0 * a16_sha)) + (2.0 * a19_sha)) + a22_sha)));
    a4_sha =
      (a11_sha +
       (1.6666666666666666e-3 *
	(((a14_sha + (2.0 * a17_sha)) + (2.0 * a20_sha)) + a23_sha)));
  }
  a2_dev[a1] = a2_sha;
  a3_dev[a1] = a3_sha;
  a4_dev[a1] = a4_sha;
