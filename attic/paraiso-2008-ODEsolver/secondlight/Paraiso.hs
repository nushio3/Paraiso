...
data Architecture = X86 | CUDA{numberOfGrids::Int, numberOfThreads::Int}
...
cuda dsl = do
  ...
  where
    docuda s = do
      ...
      do
        put s{hardware = Device fn, storeModifier = modShared, allocModifier = modShared}
        dsl 
      put s
